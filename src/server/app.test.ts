...
describe('HostelGrievance API baseline', () => {
	...
	it('forbids reading or posting comments on unauthorized grievances', async () => {
		const { cookie } = await login(app, 'student@example.test', 'student123');
		// GRV-0003 belongs to Priya (stu-2)
		const listRes = await app.request('/api/grievances/GRV-0003/comments', { headers: { Cookie: cookie } });
		expect(listRes.status).toBe(403);

		const postRes = await app.request('/api/grievances/GRV-0003/comments', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json', Cookie: cookie },
			body: JSON.stringify({ body: 'Unauthorized comment attempt' })
		});
		expect(postRes.status).toBe(403);
	});

	it('sets defensive security headers and httpOnly cookies', async () => {
		const student = await login(app, 'student@example.test', 'student123');
		expect(student.res.headers.get('x-content-type-options')).toBe('nosniff');
		expect(student.res.headers.get('x-frame-options')).toBe('DENY');
		expect(student.res.headers.get('referrer-policy')).toBe('strict-origin-when-cross-origin');
	});
});
